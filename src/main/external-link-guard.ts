 ...
 */
async function openExternalWithConfirmation(win: BrowserWindow, url: string): Promise<void> {
  if (!skipConfirmation) {
  ...
  try {
    ...
    win.webContents.send('external-link-open-failed')
  }
}
 ...
 */
export function attachExternalLinkGuard(win: BrowserWindow): void {
  win.webContents.on('will-navigate', (event, url) => {
    ...
    if (scheme === 'http') {
      void openExternalWithConfirmation(win, url)
    }
